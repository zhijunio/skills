...
写入 `TOPIC.yaml` → `depth`（默认 `standard`）。

| Level | Scope |
|-------|--------|
| **brief** | Mission + map + INDEX + 主稿精简（问题、机制、来源）；interview 仅 one-liner + ≥3 追问；可跳过 glossary / side articles / reference |
| **standard** | 完整 checklist；glossary 按需 |
| **deep** | standard + glossary 和/或 `reference/` + 0–2 分稿；机制类默认配图（见 article-structure） |

`brief` 仍须 L1/L2 与至少一项 verification；不得因「简要」跳过来源或验证门禁。
